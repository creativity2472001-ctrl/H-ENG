# main.py - الإصدار النهائي الكامل مع جميع التحسينات والتكامل (v2.2.0)
...
class StepResponse(BaseModel):
    ...
    def from_step(cls, step: Any) -> "StepResponse":
        ...
        يدعم التنسيق القديم (text, latex) والتنسيق الجديد (icon, title, content, latex)
        """
        try:
            # التعامل مع الكائنات التي تمتلك to_dict() (مثل Step من ai_engine.py)
            if hasattr(step, 'to_dict'):
                data = step.to_dict()
                
                # ✅ التنسيق الجديد من ai_engine.py v8.0 (icon, title, content, latex)
                if "icon" in data and "title" in data and "content" in data:
                    # تجميع الأيقونة والعنوان والمحتوى في وصف واحد
                    description = f"{data['icon']} **{data['title']}**: {data['content']}"
                    return cls(
                        description=description, 
                        latex=data.get("latex", "")
                    )
                
                # ✅ التنسيق القديم (text, latex)
                if "text" in data:
                    return cls(
                ...
                # ✅ التنسيق البديل (description, latex)
                return cls(
                    description=data.get("description", ""), 
                    latex=data.get("latex", "")
                )
            
            # التعامل مع tuples (للتوافق مع الإصدارات القديمة)
            elif isinstance(step, tuple) and len(step) == 2:
                return cls(description=str(step[0]), latex=str(step[1]))
            elif isinstance(step, tuple) and len(step) == 3:
                return cls(description=str(step[0]), latex=str(step[1]))
            
            # التعامل مع القواميس مباشرة
            elif isinstance(step, dict):
                # ✅ التنسيق الجديد في قاموس
                if "icon" in step and "title" in step and "content" in step:
                    description = f"{step['icon']} **{step['title']}**: {step['content']}"
                    return cls(
                        description=description, 
                        latex=step.get("latex", "")
                    )
                # ✅ التنسيق القديم في قاموس
                return cls(
                    description=step.get("text", step.get("description", "")), 
                    latex=step.get("latex", "")
                )
            
            # أي شيء آخر - نحوله لنص
            else:
                return cls(description=str(step), latex="")
                
        except Exception as e:
            # في حالة حدوث خطأ، نعيد خطوة تحتوي على وصف الخطأ
            return cls(
                description=f"⚠️ خطأ في تحويل الخطوة: {str(e)}", 
                latex=""
            )
...
async def lifespan(app: FastAPI):
    ...
    if hasattr(app.state.ai_engine, 'set_math_engine'):
        app.state.ai_engine.set_math_engine(app.state.math_engine)
        print("✅ AI Engine مرتبط مع Math Engine")
    else:
        print("ℹ️ AI Engine لا يحتاج ربط (موحد)")
