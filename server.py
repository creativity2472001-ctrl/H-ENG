...
# استيراد محرك الخطوات (جديد)
from solver.steps_engine import solve_with_steps as generate_steps
...
async def calculate(data: ExpressionRequest):  
    """نقطة النهاية الرئيسية (نتيجة فقط)"""  
    return solve_calculate(data.expression)  

# ===== نقطة نهاية جديدة للخطوات =====  
@app.post("/solve")  
async def solve(data: ExpressionRequest):  
    """نقطة نهاية لعرض الحل مع الخطوات التفصيلية"""  
    try:  
        logger.info(f"طلب حل مع خطوات: {data.expression}")  
        result = generate_steps(data.expression)  
        
        # التأكد من وجود الحقول المطلوبة
        if not isinstance(result, dict):
            result = {"success": False, "error": "نتيجة غير صالحة"}
        
        # إضافة الحقول الافتراضية إذا لم تكن موجودة
...
if __name__ == "__main__":  
    ...
    print("   ✅ POST /calculate - نتيجة فقط")  
    print("   ✅ POST /solve     - نتيجة + خطوات مفصلة")  
    print("   ✅ POST /algebra/template/{id} - قالب محدد")  
    print("   ✅ POST /algebra/system - نظم معادلات")  
    print("="*70)  
