# server.py - خادم الويب (معدل مع دعم القوالب الجديدة ونظام المعادلات)
...
def parse_system_equations(expr: str):
    """تحليل صيغة نظام المعادلات مثل [2*x + 3*y = 8, 3*x - y = 1]"""
    try:
...
def format_system_result(result):
    ...
    try:
        ...
        # التحقق من وجود حل في template_21_system_2x2_unique
        ...
        # التحقق من وجود حل في template_22_system_2x2_infinite
        if 'is_dependent' in result and result['is_dependent']:
            return f"عدد لا نهائي من الحلول: {result.get('general_solution', '')}"
        
        # التحقق من وجود حل في template_23_system_2x2_no_solution
        if 'is_inconsistent' in result and result['is_inconsistent']:
            return "لا يوجد حل (نظام متناقض)"
        
        # محاولة استخراج الحل من الصيغ الأخرى
                ...
                if 'sympy_method' in result['solutions']:
                    ...
                    if sol:
                        ...
                        if 'x' in sol and sol['x'] is not None:
                            parts.append(f"x = {sol['x']}")
                        if 'y' in sol and sol['y'] is not None:
                            parts.append(f"y = {sol['y']}")
                        if 'z' in sol and sol['z'] is not None:
                            parts.append(f"z = {sol['z']}")
                        if parts:
                            return ", ".join(parts)
...
async def calculate(data: ExpressionRequest):
    try:
        ...
        # ===== 0. التعامل مع نظم المعادلات =====
        if expr.startswith('[') and expr.endswith(']') and ',' in expr:
            system = parse_system_equations(expr)
            if system:
                if system["type"] == "system_2x2":
                    # محاولة استخدام التوابع الصحيحة من solver1
                    try:
                        # أولاً: محاولة template_21 (حل وحيد)
                        result = solver1.template_21_system_2x2_unique(
                            system["eq1"], system["eq2"]
                        )
                        formatted = format_system_result(result)
                        return {"success": True, "result": formatted}
                    except AttributeError:
                        try:
                            # ثانياً: محاولة template_22 (عدد لا نهائي)
                            result = solver1.template_22_system_2x2_infinite(
                                system["eq1"], system["eq2"]
                            )
                            formatted = format_system_result(result)
                            return {"success": True, "result": formatted}
                            ...
                            try:
                                # ثالثاً: محاولة template_23 (لا يوجد حل)
                                result = solver1.template_23_system_2x2_no_solution(
                                    system["eq1"], system["eq2"]
                                )
                                formatted = format_system_result(result)
                                return {"success": True, "result": formatted}
                            except AttributeError:
                                # رابعاً: محاولة template_04 (الاسم القديم)
                                try:
                                    ...
                                    return {"success": False, "error": "لم يتم العثور على دالة نظام المعادلات في solver1"}
                
                elif system["type"] == "system_3x3":
                    try:
                        # محاولة template_27 (نظام 3×3)
                        # نحتاج لاستخراج المعاملات من المعادلات
                        # هذا تبسيط - في التطبيق الفعلي نحتاج لتحليل المعادلات
                        result = solver1.template_27_system_3x3_unique(
                            system["eq1"], system["eq2"], system["eq3"]
                        )
                        return {"success": True, "result": str(result)}
                    except AttributeError:
                        try:
                            ...
                            return {"success": False, "error": "لم يتم العثور على دالة نظام 3×3"}
        ...
        # ===== 3. التعامل مع اللوغاريتمات بالصيغة log(x, base) =====
        log_match = re.search(r'log\(([^,]+),\s*(\d+)\)', expr)
        if log_match and '=' in expr:
            # هذا تعبير لوغاريتمي سيتم التعامل معه بواسطة solve_equation
            pass
        
        # ===== 4. التعامل مع المعادلات البسيطة (تحتوي على =) =====
        if '=' in expr:
    ...
    try:
        ...
        if 1 <= template_id <= 52:
            if template_id == 1:
                ...
                # نظام 2×2 - نحاول التوابع المختلفة
                try:
                    result = solver1.template_21_system_2x2_unique(
                    ...
                    try:
                        ...
                        result = {"error": "قالب نظام 2×2 غير موجود"}
            elif template_id == 5:
                # نظام 3×3
                try:
                    result = solver1.template_27_system_3x3_unique(
                    ...
                    try:
                        ...
                        result = {"error": "قالب نظام 3×3 غير موجود"}
            elif template_id == 6:
                ...
                result = {"error": f"القالب {template_id} غير مضمن بعد"}
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("✅ يدعم الآن صيغة نظام المعادلات: [2*x + 3*y = 8, 3*x - y = 1]")
    print("✅ تم تصحيح أسماء التوابع (template_21, template_22, template_23)")
    print("="*60)
